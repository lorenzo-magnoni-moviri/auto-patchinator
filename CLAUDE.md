...
- The tool runs natively on Windows too (no WSL needed) — there's no POSIX-only code (`pty`/`fcntl`/`termios`/`os.fork` etc.) anywhere, and every SSH command is a string addressed to the *remote* Linux hosts, so it's irrelevant to the local OS. Keep it that way: avoid introducing local shell-outs or POSIX-only stdlib calls (see README's "Running on Windows" for the current setup differences, which are CLI-invocation-only, not code-level).
